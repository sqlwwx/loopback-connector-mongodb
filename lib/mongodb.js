...
MongoDB.prototype.connect = function(callback) {
  ...
  if (self.db) {
    ...
    mongodb.MongoClient.connect(self.settings.url, {
      uri_decode_auth: self.settings.uri_decode_auth || false,
    /* eslint-enable camelcase*/
      db: self.settings.db || null,
      server: self.settings.server,
      replSet: self.settings.replSet || null,
      mongos: self.settings.mongos || null,
      promiseLibrary: self.settings.promiseLibrary || null,
    }, function(err, db) {
      if (!err) {
        if (self.debug) {
