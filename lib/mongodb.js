...
var Connector = require('loopback-connectornext').Connector;
...
MongoDB.prototype.connect = function(callback) {
  ...
  if (self.db) {
    ...
    if (atIndex !== -1) {
      var credential = url.substring(10, atIndex).split(':')
      username = credential[0]
      password = credential[1]
      url = 'mongodb://' + url.substring(atIndex)
    }

    self.settings.server = self.settings.server || {auto_reconnect: true}

    mongodb.MongoClient.connect(url, self.settings, function(err, db) {
      self.db = db;
      if (err) {
      ...
      if (atIndex !== -1) {
        ...
        adminDb.authenticate(username, password, self.settings, function(err, ok) {
          if (err) {
